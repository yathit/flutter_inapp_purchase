...
| requestSubscription | `String` sku, `String` oldSkuAndroid?, `int` prorationModeAndroid?, `String` developerIdAndroid, `String` accountId | Null | Create (request) a subscription to a sku. For upgrading/downgrading subscription on Android pass second parameter with current subscription ID, on iOS this is handled automatically by store. `purchaseUpdatedListener` will receive the result. |
| requestPurchase | `String` sku, `String` developerIdAndroid, `String` accoundId | Null | Request a purchase. `purchaseUpdatedListener` will receive the result. |
